...
using Cargill.DUE.Web.ServicoSiscomex;
        ...
        public static async Task<RetornoSiscomex> EnviarSemServico(string xml, string due, string cpfCertificado)
        {
            try
            {
                try
                {
                    ...
                    var retornoSiscomex = new Cargill.DUE.Web.ServicoSiscomex.RetornoSiscomex();
                    ...
                    using (MemoryStream memoryStream = new MemoryStream(Encoding.UTF8.GetBytes(Regex.Unescape(retornoResponse))))
                    {
                        ...
                        if (response.StatusCode != HttpStatusCode.OK)
                        {
                            try
                            {
                                ...
                                var criticas = (Cargill.DUE.Web.ServicoSiscomex.RetornoCriticas)xmlSerializer.Deserialize(reader);
